// GUIDO — Edge Function: envío de correo vía Gmail SMTP
// Supabase · Deno · nodemailer
// Deploy: supabase functions deploy enviar-correo
// Secret:  supabase secrets set GMAIL_APP_PASSWORD="gslygoznfkcudwkc"
...
serve(async (req: Request) => {
  ...
  try {
    const { to_email, to_name, reset_link, expires_in = "1 hora" } =
      await req.json();
    ...
    const transporter = nodemailer.createTransport({
      ...
      auth: {
        ...
        pass: Deno.env.get("GMAIL_APP_PASSWORD"),   // secreto en Supabase
      },
    });

    // ── HTML del correo con branding GUIDO ────────────
...
<table width="100%" cellpadding="0" cellspacing="0"
  style="max-width:580px;background:#fff;border-radius:14px;overflow:hidden;
         box-shadow:0 6px 32px rgba(7,26,68,.14);">

  <!-- ENCABEZADO NAVY -->
  <tr><td style="background:#071a44;padding:36px 40px 28px;text-align:center;">
    <div style="font-size:11px;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:#c8102e;margin-bottom:10px;">
      República Dominicana
    </div>
    <div style="font-size:48px;font-weight:900;color:#fff;letter-spacing:.04em;line-height:1;">GUIDO</div>
    <div style="font-size:12px;color:rgba(255,255,255,.55);margin-top:6px;letter-spacing:.1em;text-transform:uppercase;">
      Siempre cerca de ti
    </div>
  ...
  <tr><td style="padding:40px 44px 32px;">
    <p style="font-size:16px;color:#1a2a4a;margin:0 0 18px;font-weight:700;">
      Hola, ${to_name ?? "usuario"}
    </p>
    ...
    <tr><td align="center" style="padding:4px 0 36px;">
      <a href="${reset_link}"
         style="display:inline-block;padding:16px 44px;background:#c8102e;color:#fff;
                font-size:15px;font-weight:700;text-decoration:none;border-radius:50px;
                letter-spacing:.04em;">
    ...
    <p style="font-size:11px;color:#a0aec0;word-break:break-all;margin:0 0 32px;
              background:#f7f9fc;padding:12px 16px;border-radius:8px;
    ...
    <tr><td style="background:#fff8f8;border:1px solid #fde8e8;border-radius:8px;padding:14px 18px;">
      <p style="font-size:13px;color:#b91c1c;margin:0;">
        ⏱ <strong>Este enlace expira en ${expires_in}.</strong>
  ...
  <tr><td style="background:#071a44;padding:24px 40px;text-align:center;">
    <p style="font-size:12px;color:rgba(255,255,255,.6);margin:0 0 4px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;">
      GUIDO — Sistema de Gestión Territorial
    </p>
    <p style="font-size:11px;color:rgba(255,255,255,.35);margin:0;">
      República Dominicana · Siempre cerca de ti
    </p>
    <div style="width:32px;height:2px;background:#c8102e;margin:14px auto 0;border-radius:1px;"></div>
    <p style="font-size:10px;color:rgba(255,255,255,.25);margin:12px 0 0;">
